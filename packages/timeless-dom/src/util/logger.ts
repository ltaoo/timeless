...
function buildLogParts(props: LoggerProps, level: LogLevel): string[] {
  ...
  if (mode === "verbose") {
    ...
    styles.push(
      `color:white;background:${prefixBg};border-top-left-radius:2px;border-bottom-left-radius:2px;`,
    ...
    styles.push(
      `color:white;background:${prefixBg};border-radius:2px;`,
...
export function Logger(props: LoggerProps = {}) {
  ...
  return {
    ...
    ...logMethods,
    log: logMethods.info,
    setPrefixColor(color: string) {
      currentProps = { ...currentProps, prefixColor: color };
    },
  };
}
