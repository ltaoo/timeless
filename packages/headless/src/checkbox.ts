...
export function Checkbox(props: ViewProps & { store?: any, theme?: any }) {
  ...
  return View({
    ...
    View({
      class: computed({ state }, (d: any) => merge(tp(t?.box, { checked: d.state.checked, disabled: d.state.disabled })).class || ""),
      style: computed({ state }, (d: any) => merge(tp(t?.box, { checked: d.state.checked, disabled: d.state.disabled })).style || ""),
    }, [
      View({
        type: "span",
        class: computed({ state }, (d: any) => merge(tp(t?.check, { checked: d.state.checked })).class || ""),
        style: computed({ state }, (d: any) => merge(tp(t?.check, { checked: d.state.checked })).style || ""),
