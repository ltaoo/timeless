...
export function Button(props: ViewProps & { store?: any, theme?: any, variant?: string, size?: string }, children?: any) {
  ...
  if (store) {
    ...
    const m = (d: any) => merge(tp(t?.root, { variant, size, loading: d.state.loading, disabled: d.state.disabled }), cn, st);
