 ...
 *
 * 这是 @timeless/unittest 的核心测试：验证跨组件协调的正确性。
 *
 * 位置体系：
 * - Window 绝对坐标原点在 (100, 80)，客户区 480×360
 * - platform 单例提供 getBoundingClientRect / getViewportSize 等 API
 * - PopperCore 通过 platform 获取节点绝对位置，计算出下拉项的 style（x/y/top/bottom/...）
 */
...
describe("完整场景：键盘+鼠标操作 → 每一步全状态断言", () => {
  // —— Window：所有组件都放在 Window 中，Window 就是 host window ——
  ...
  // —— platform：直接从 Window 获取视口尺寸，无需 mock ——
  // item-aligned 模式下 PopperCore 只用 platform.getViewportSize()
  setPopperPlatform({
    getViewportSize: () => win.state.size,
  } as Parameters<typeof setPopperPlatform>[0]);
  ...
  const select = new SelectCore<string>({
    ...
    placeholder: "请选择",
    options: [
      new SelectItemCore({ value: "bj", label: "北京" }),
      new SelectItemCore({ value: "sh", label: "上海" }),
      new SelectItemCore({ value: "sz", label: "深圳" }),
      (() => {
        const item = new SelectItemCore({ value: "gz", label: "广州" });
        item.disabled = true;
        return item;
      })(),
    ],
  ...
  // —— Select trigger 在 body 内的局部坐标 ——
  // Window 位于 (100, 80)，header 高 40px，trigger 在 body 内 (40, 80)
  // getAbsoluteRect 自动换算为绝对坐标
  const triggerRect = win.methods.getAbsoluteRect({ x: 40, y: 80, width: 200, height: 40 });
  // 下拉内容的绝对尺寸
  const floatingRect = { x: 0, y: 0, width: 200, height: 200, top: 0, left: 0, right: 200, bottom: 200 } as const;
  ...
  it("步骤 0：初始状态——所有组件已创建，弹窗关闭，下拉未展开", () => {
    // —— Window 位置 ——
    expect(win.state.position).toEqual({ x: 100, y: 80 });
    expect(win.state.size).toEqual({ width: 480, height: 360 });
    // body 绝对 rect：window.y + header(40px)，window.height - header - footer(48px, 默认可见)
    expect(win.state.body.rect).toMatchObject({
      y: 80 + 40,
      height: 360 - 40 - 48,
    });
  ...
  it("步骤 3：模拟 click Select 触发器 → 下拉展开，模拟渲染层挂载 reference/floating → 断言下拉项绝对位置", () => {
    ...
    // —— 模拟渲染层：挂载 trigger（reference）和下拉内容（floating） ——
    // 真实 DOM 中渲染层通过 platform.getBoundingClientRect() 获取节点绝对位置，
    // 然后调用 popper$.setReference()/setFloating()，触发位置计算。
    // 这里直接用已知的 Window 内绝对坐标构造 Rect 来驱动 PopperCore。
    select.popper$.setReference({ getRect: () => triggerRect });
    select.popper$.setFloating({ getRect: () => floatingRect });

    // item-aligned 模式下由 DOM 层传入选中项偏移和内容度量，直接同步计算
    select.popper$.adjustContentPositionWithOffsetTop({
      selectedItem: {
        offsetTop: 0,
        offsetHeight: 32,
        bottom: 32,
    ...
    // —— 下拉项定位 style ——
    // PopperCore.state 即为渲染层的 CSS 数据源：
    // item-aligned 模式 → position:fixed, left:<x>px, top/bottom:<top/bottom>px, min-width:<minWidth>px, height:<height>px
    const popper3 = snapPopper(select.popper$);
    // reference 已挂载
    expect(popper3.reference).toBe(true);
    // 水平方向：left = reference.left + 1，与 trigger 左对齐
    expect(popper3.x).toBe(triggerRect.left + 1);
    // min-width 匹配 trigger 宽度
    expect(popper3.minWidth).toBe(triggerRect.width - 1);
    // item-aligned 落在 trigger 下方（top 对齐时 bottom=0）
    expect(popper3.bottom).toBe(0);
    // 高度已计算（由 adjustContentPositionWithOffsetTop 根据 viewport$/floating/reference 度量同步算出）
    expect(popper3.height).toBe(232);
    // max-height 受 viewport 可用空间限制：window.height - margin*2 = 340
    expect(popper3.maxHeight).toBe(340);
    // margin 用于与窗口边界保持间距
    expect(popper3.margin).toBe(10);
    // placement 默认 bottom
    expect(popper3.placement).toBe("bottom");
  ...
  it("步骤 4：键盘 ↓ → focusNextOption → 聚焦第 1 个（跳过禁用的 gz）", () => {
    ...
    // 聚焦第一个非禁用选项 "北京"
    expect(snap4.focusedItem).toMatchObject({
      value: "bj",
      label: "北京",
      focused: true,
    });
    ...
    // 选项级别验证
    expect(snap4.options[0]).toMatchObject({
      value: "bj",
      focused: true,
      selected: false,
    });
    ...
    expect(snap4.options[3]).toMatchObject({
      value: "gz",
      focused: false,
      disabled: true,
    });
  ...
  it("步骤 5：键盘 ↓ → focusNextOption → 聚焦第 2 个", () => {
    ...
    expect(snap5.focusedItem).toMatchObject({
      value: "sh",
      label: "上海",
      focused: true,
    });
  ...
  it("步骤 6：键盘 ↓ → focusNextOption → 聚焦第 3 个", () => {
    ...
    // 跳过禁用的 gz，命中 "深圳"
    expect(snap6.focusedItem).toMatchObject({
      value: "sz",
      label: "深圳",
      focused: true,
    });
    ...
    expect(snap6.options[3]).toMatchObject({
      value: "gz",
      focused: false,
      disabled: true,
    });
  ...
  it("步骤 7：键盘 ↓ → 循环回到第 1 个（跳过禁用的 gz）", () => {
    ...
    expect(snap7.focusedItem).toMatchObject({
      value: "bj",
      label: "北京",
      focused: true,
    });
  ...
  it("步骤 8：键盘 ↑ → focusPrevOption → 回到第 3 个", () => {
    ...
    expect(snap8.focusedItem).toMatchObject({
      value: "sz",
      label: "深圳",
      focused: true,
    });
