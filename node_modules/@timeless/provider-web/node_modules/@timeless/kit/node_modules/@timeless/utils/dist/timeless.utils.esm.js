...
function parseJSONStr(json) {
	try {
		if (json[0] !== "{" && json[0] !== "[") return Result.Err("不是合法的 json");
...
function readFileAsURL(file) {
	return new Promise((resolve) => {
		const reader = new FileReader();
		reader.onload = (e) => {
			if (e.target === null) return resolve(Result.Err("read failed"));
			resolve(Result.Ok(reader.result));
		};
...
export { blobToArrayBuffer, chinese_num_to_num, debounce, downloadFile, inRange, loadImage, num_to_chinese, padding_zero, parseJSONStr, qs_parse, qs_stringify, random_key, readFileAsArrayBuffer, readFileAsURL, remove_arr_item, remove_str, sleep, throttle, toFixed, toNumber, uidFactory, update_arr_item };

//# sourceMappingURL=timeless.utils.esm.js.map