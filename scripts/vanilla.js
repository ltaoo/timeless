...
function startDev() {
  ...
  try {
    ...
    packages.forEach((pkg) => {
      ...
      if (fs.existsSync(srcPath)) {
        // Only watch packages relevant to our artifacts or dependencies?
        // Ideally we should watch all, because changing a dependency should trigger rebuild.
...
function startServer() {
  ...
  const port = portArg ? parseInt(portArg.split("=")[1], 10) : 3001; // Default to 3001 to avoid conflict with dev.js
  ...
  const server = http.createServer((req, res) => {
    ...
    if (targetPath.startsWith(prefix)) {
      ...
      // Allow loading root files if needed, or redirect?
      // For consistency with dev.js, we enforce /timeless prefix.
      res.statusCode = 404;
