...
// }
var APIHostname = "http://100.78.198.69:2022";
...
function DownloaderPanelViewModel() {
  ...
  let _scrollTop = 0;
  // ---- 模拟下载任务 ----
  const fakeTaskTotal = 4718967;
  let fakeDownloaded = 0;
  let fakeStatus = "running";
  let fakeUsed = 0;
  const fakeStartTime = new Date().toISOString();
  function buildFakeTask() {
    return {
      id: "YCK3K5pWD1d9Ma_jt267e",
      protocol: "http",
      meta: {
        req: {
          url: "https://example.com/fake-download",
          extra: null,
          labels: {
            id: "14875682446822279313",
            key: "99062626",
            nonce_id: "",
            spec: "xWT111",
            suffix: ".mp4",
            title:
              "神子的语录：没有人会一直爱你\n#八重神子 #原神 #cosplay #语录 #好身材\n",
          },
          proxy: null,
        ...
        opts: {
          name: "神子的语录：没有人会一直爱你#八重神子 #原神 #cosplay #语录 #好身材_xWT111.mp4",
          path: "/volume1/homes/ltaoo/Applications/WXChannelsDownload/Downloads",
          selectFiles: [],
          extra: {
            connections: 4,
          },
      ...
      progress: {
        used: fakeUsed,
        speed:
          fakeStatus === "running" ? (800000 + Math.random() * 400000) | 0 : 0,
  ...
  const timer = setInterval(() => {
    if (fakeStatus === "running") {
      // 每次增加一部分，模拟下载进度
      var chunk = (80000 + Math.random() * 60000) | 0;
      fakeDownloaded = Math.min(fakeDownloaded + chunk, fakeTaskTotal);
      fakeUsed += 200;
      if (fakeDownloaded >= fakeTaskTotal) {
        fakeStatus = "done";
        clearInterval(timer);
      }
    }
    methods.upsert(methods.formatTask(buildFakeTask()));
  }, 200);
  const ui = {
    ...
    [
      ...
      View(
        ...
        [
          ...
          //     each: tasks_,
          //     render(task) {
          //       return View({}, [computed(task, (t) => t.name)]);
          //     },
          //   }),
          ScrollView(
            {
...
(() => {
  ...
  setTimeout(() => {
    ...
    console.log("[DOWNLOADER]insert_downloader", $header);
    if (!$header) return;
