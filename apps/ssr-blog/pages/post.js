import { View, Txt, For } from "@timeless/timeless";
...
// Same post data — in a real app this would come from a database or CMS
const posts = [
  {
    slug: "getting-started-with-timeless",
    title: "Getting Started with Timeless",
    excerpt:
      "Learn how to build reactive web applications with the Timeless framework.",
    date: "2025-04-10",
  ...
  },
  {
    ...
    excerpt:
      "Deep dive into server-side rendering with Timeless.",
    date: "2025-04-08",
    author: "Timeless Team",
    tags: ["ssr", "advanced"],
    content: [
  ...
  },
  {
    ...
    excerpt:
      "Explore the Shadcn component library for Timeless.",
    date: "2025-04-05",
    author: "Timeless Team",
  ...
  },
  {
    ...
    excerpt:
      "Master reactive programming patterns in Timeless.",
    date: "2025-04-02",
    author: "Timeless Team",
  ...
  },
  {
    ...
    excerpt:
      "A complete guide to building and deploying Timeless applications to production.",
    date: "2025-03-28",
    author: "Timeless Team",
...
export async function load({ query }) {
  ...
  const index = findPostIndex(slug);
...
export function head({ data }) {
  ...
  if (!post) {
    return {
      title: "Post Not Found — Timeless Blog",
...
function renderBlock(block) {
  if (block.type === "heading") {
    return View({ as: "h2" }, [block.text]);
  }
  if (block.type === "code") {
    return View({ as: "pre" }, [View({ as: "code" }, [block.text])]);
  }
  // paragraph
  return View({ as: "p" }, [block.text]);
}
...
export default function Page({ data }) {
  ...
  if (!post) {
    return View({ class: "min-h-screen bg-background" }, [
      View({ class: "max-w-3xl mx-auto px-6 py-20 text-center" }, [
        View({ as: "h1", class: "text-3xl font-bold text-foreground" }, [
          "Post Not Found",
        ]),
        View({ as: "p", class: "mt-4 text-muted-foreground" }, [
  ...
  return View({ class: "min-h-screen bg-background" }, [
    // Back navigation
    View({ as: "nav", class: "border-b border-border" }, [
      View({ class: "max-w-3xl mx-auto px-6 py-4" }, [
        View(
          {
            as: "a",
            href: "/",
    ...
    // Article
    View({ as: "article", class: "max-w-3xl mx-auto px-6 py-12" }, [
      // Header
      View({ as: "header", class: "mb-8" }, [
        // Tags
        ...
        // Meta
        View({ class: "mt-4 flex items-center gap-3 text-sm text-muted-foreground" }, [
          View({ as: "span" }, [
            Txt("By "),
      ...
      View({ class: "flex items-center justify-between" }, [
        ...
        data.prevPost
          ? View({ class: "flex flex-col items-start" }, [
              View({ as: "span", class: "text-xs text-muted-foreground mb-1" }, [
                "Previous",
              ]),
              View(
                {
                  as: "a",
                  href: `/post?slug=${data.prevPost.slug}`,
        ...
        data.nextPost
          ? View({ class: "flex flex-col items-end" }, [
              ...
              View(
                {
                  as: "a",
                  href: `/post?slug=${data.nextPost.slug}`,
                  class: "text-sm font-medium text-primary hover:underline",
                },
                [`${data.nextPost.title} →`],
              ),
    ...
    // Footer
    View({ as: "footer", class: "border-t border-border mt-8" }, [
