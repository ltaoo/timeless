const { View, Text, For, ref, refobj, computed } = Timeless;
...
export default function ArticleListPageView(props) {
  ...
  const syncFromQuery = (q = {}) => {
    ...
    if (!exists) {
      curId.as(first.id);
      props.history.replace("root.home_layout.article.category.detail", {
        ...routeQuery.value,
        cate_id: nextCateId,
        id: first.id,
      });
  ...
  props.history.onRouteChange(({ view, query }) => {
    ...
    if (!name.startsWith("root.home_layout.article.")) return;
    routeQuery.as({ ...(query || {}) });
    syncFromQuery(routeQuery.value);
  });
        ...
        View({ class: "flex-1 overflow-y-auto py-2" }, [
          For({
            ...
            render(article) {
              return View(
                {
                  class: "mx-2 px-3 py-2.5 rounded-md cursor-pointer transition-colors " +
                    (curId.value === article.id
                      ? "bg-zinc-100 dark:bg-zinc-800"
                      : "hover:bg-zinc-50 dark:hover:bg-zinc-900"),
                  onClick() {
                    curId.as(article.id);
                    props.history.push(
                      "root.home_layout.article.category.detail",
                ...
                [
                  View(
                    {
                      class: "text-sm leading-relaxed line-clamp-1 " +
                        (curId.value === article.id
