const { View, Text, Show, ref, refobj, computed } = Timeless;
        ...
        return [
          View({ class: "flex items-center justify-center h-full" }, [
            View({ class: "flex flex-col items-center gap-3" }, [
              View({
                class: "w-6 h-6 border-2 border-zinc-300 border-t-zinc-600 rounded-full animate-spin dark:border-zinc-600 dark:border-t-zinc-300",
              }),
...
function ArticleContent(props) {
  return View({ class: "max-w-2xl mx-auto px-8 py-10" }, [
    View({ class: "mb-8" }, [
      View(
        {
          class: "text-2xl font-bold text-zinc-900 leading-tight dark:text-zinc-100",
        },
        [props.data.title],
      ),
    ]),
    View({ class: "border-t border-zinc-200 dark:border-zinc-800" }),
    View({ class: "mt-6" }, [
      View(
        {
